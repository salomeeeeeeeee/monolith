...
setInterval(() => {
	...
	let pop = document.getElementById("crm-card-extended-info");

	if (pop && pop.children.length >= 3) {
		// pop.children[1].style.display = "none"; 
		pop.children[2].style.display = "none"; // Responsible person
	}

	const crmButtonsContainer = document.querySelector('.im-phone-call-crm-buttons');

	if (crmButtonsContainer) {
		crmButtonsContainer.style.display = "none"; // hide COMMENT / DEAL / INVOICE row entirely
	}
