...
function getLeadByFilter($arFilter) {
    $res = CCrmLead::GetListEx(array("ID"=>"DESC"), $arFilter, false, Array("nPageSize"=>1), array());
    if($arLead = $res->Fetch()) {
...
function checkLeads($mobileNumber, $leadId) {
    ...
    $dbFieldMulti = \CCrmFieldMulti::GetList(array(), array('ENTITY_ID' => 'LEAD','TYPE_ID' => 'PHONE', "%VALUE" => $mobileNumber));
